...
const EditProject = require('../../../classes/project/editProject');
const InviteUser = require('../../../classes/user/inviteUser');
const CreateFloor = require('../../../classes/view/floor/createFloor');
...
describe('Company management tests @Sb36e9099', async () => {
  ...
  const companyCreate = 'focus on the first element of the company creation form';
  const projectCreate = 'focus on the first element of the project creation form';
  ...
  it('Verification about cursor in first field of item creation forms @T36d81540', async () => {
    ...
    try {
      ...
      await createRoom.clickElement(
        config.locatorAddRoomBtnCss,
        false,
        0
      );
      await createRoom.clickElement(config.locatorCreateRoomBtnCss);
      await createRoom.isFirstInputFocused(
        '.form-invite',
        roomCreate
      );
      ...
      await createTask.isFirstInputFocused(
        'app-task-form',
        taskCreate
      );
  ...
  it('Focus to first field Edit User Form', async () => {
    await lambdaParameters(
      'Focus to first field Edit User Form',
      driverChrome
    );
    // time and site or lochalhost there tests are going
    console.log(Date().toLocaleLowerCase(), 'date', config.urlLoginPage);
  ...
  it('Focus to first field Edit Project Form', async () => {
    ...
    try {
      ...
      await driverChrome.sleep(1000);
      await editProject.findProject(config.projectNameEdit, config.projectsPage);
