...
describe('create and remove project in the chrome browser', async () => {
  ...
  const projectsPage =
    'https://dev-frontend.colorjob.terenbro.com/system/projects';
  ...
  it('create new project', async () => {
    ...
    for (index = 0; index < 20; ++index) {
      ...
      const projectNumber = await driverChrome.findElement(
        By.id('projectNumber')
      );
      await projectNumber.sendKeys(namber);

      const companyProjectBelong = await driverChrome.findElement(
      ...
      const companyList = await driverChrome.findElements(
        By.className('ng-option')
      );

      await CreateProject.findDateInDropDown(companyList, company);

      const addressStreet = await driverChrome.findElement(
        By.id('projectAddress')
      );
      await addressStreet.sendKeys(street);

      const addressApart = await driverChrome.findElement(
        By.id('projectAddressSecond')
      );
      await addressApart.sendKeys(app);

      const stateDropDown = await driverChrome.findElement(
        By.id('projectSelectState')
      );
      await stateDropDown.click();
      const stateList = await driverChrome.findElements(
        By.className('ng-option')
      );

      await CreateProject.findDateInDropDown(stateList, state);

      const cityDropDown = await driverChrome.findElement(
        By.id('projectSelectCity')
      );
      await cityDropDown.click();
      const cityList = await driverChrome.findElements(
        By.className('ng-option')
      );
      await CreateProject.findDateInDropDown(cityList, city);

      const projectZip = await driverChrome.findElement(
