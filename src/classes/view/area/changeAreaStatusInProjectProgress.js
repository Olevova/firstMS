...
class ChangeAreaStatusInProjectProgress extends ChangeAreaStatus {
  ...
  async hoverAreaAndCheckCommentsAndAttachmentsNew() {
    ...
    await this.driver.wait(until.elementLocated(By.css('.area-progress-list-areas__item [hascomments="true"]')),10000);
    const areaElementWithAttachmentAndComment = await this.driver.findElement(
      By.css(`.area-progress-list-areas__item [hascomments="true"]`)
    );
    const attachmenTrue = await this.driver.findElement(By.css(`.area-progress-list-areas__item [hasattachments="true"]`)).catch(()=>null);
